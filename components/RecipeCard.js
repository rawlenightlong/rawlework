...
return (
            <ScrollView contentContainerStyle={{flexGrow: 0}} style={{ padding: 10, backgroundColor: "#1976d2", width: "85%", borderRadius: 15}}>
                <View style={{padding: 15, alignItems: "center"}}>
                    ...
                    <View style={{margin: 5, marginBottom: 15}}>
                            {ingredients.map((ingredient, index) => <Text key={index} style={{textAlign: 'center', margin: 2, fontStyle: "italic", color: "white"}}>{" ~"}{ingredient}</Text>)}
                    </View>
                    ...
                    <View style={{marginBottom:10}}>
                        {/* {instructions} */}
                        {instructions.map((instruction, index) => <Text key={index} style={{margin: 8, color: "white"}}>{instruction}</Text>)}
                    </View>
